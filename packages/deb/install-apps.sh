...
mozilla_source="deb [signed-by=/etc/apt/keyrings/packages.mozilla.org.asc] https://packages.mozilla.org/apt mozilla main"
if ! grep -q "$mozilla_source" /etc/apt/sources.list.d/mozilla.list; then
  echo "$mozilla_source" | sudo tee -a /etc/apt/sources.list.d/mozilla.list >/dev/null
fi
echo '
Package: *
